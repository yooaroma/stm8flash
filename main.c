...
bool usb_init(programmer_t *pgm, bool pgm_serialno_specified, char *pgm_serialno)
{
	...
	for (int i = 0; i < cnt; i++) 
	{
		...
		int32_t ret = libusb_get_device_descriptor(devs[i], &desc);
		if (ret < 0)
		{
			fprintf(stderr, "failed to get libusb device descriptor (libusb error: %d)\n", ret);
			break;
		}
		// fprintf(stderr, "vid[0x%04X],pid[0x%04X]\n",desc.idVendor,desc.idProduct);
		if (desc.idVendor == pgm->usb_vid && desc.idProduct == pgm->usb_pid)
		{
			if(desc.idVendor == 0x0483 && desc.idProduct == 0x3744)
			{
				desc.bDeviceClass = LIBUSB_CLASS_MASS_STORAGE; 	/** Mass storage class */ //	LIBUSB_CLASS_MASS_STORAGE = 0x08,
			}
			fprintf(stderr, "ok:vid[0x%04X],pid[0x%04X],bDeviceClass[%d],size(%lld) (bus %d, device %d) \n", desc.idVendor, desc.idProduct,desc.bDeviceClass,sizeof(desc),libusb_get_bus_number(*devs), libusb_get_device_address(*devs));
			DumpHex((const void *)&desc, sizeof(desc));
			numOfProgrammers++;
		}
	}
	...
	if (numOfProgrammers > 1 || pgm_serialno_specified)
	{
		...
		for (i = 0; i < cnt; i++)
		{
			...
			// fprintf(stderr, "ok:vid[0x%04X],pid[0x%04X],size(%lld)\n", desc.idVendor, desc.idProduct,sizeof(desc));
			// DumpHex((const void *)&desc, sizeof(desc));
			if (desc.idVendor == pgm->usb_vid && desc.idProduct == pgm->usb_pid)
			{
				fprintf(stderr, "ok:vid[0x%04X],pid[0x%04X],size(%lld)\n", desc.idVendor, desc.idProduct, sizeof(desc));
...
int main(int argc, char **argv)
{
	...
	case FLASH:
		...
		fprintf(stderr, "Determine FLASH area start[0x%X]:size[%d] bytes\r\n", start, bytes_count);
		break;
	case OPT:
		if (!start_addr_specified)
		{
